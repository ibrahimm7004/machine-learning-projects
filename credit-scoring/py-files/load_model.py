...
def initial_tasks(directory, dataset_to_test):
    # Read the CSV file
    df = pd.read_csv(os.path.join(directory, 'excel_files', dataset_to_test))
...
def load_preprocessors(directory):
    ...
    encoder = joblib.load(os.path.join(directory, 'model_files/encoder.joblib'))
...
def apply_model(df, directory, original_dataset):
    ...
    loaded_model = joblib.load(os.path.join(new_directory, '{}.joblib'.format(name)))  
    ...
    original_dataset.to_csv(os.path.join(directory, 'excel_files/predicted_data.csv'), index=False)
...
def print_comparison(directory):
    ...
    df1 = df1.head(10)[['credit_score']].rename(columns={'credit_score': 'Original'})
    df2 = df2.head(10)[['credit_score']].rename(columns={'credit_score': 'Predicted'})
...
def main():
    directory = r'C:\Users\hp\Desktop\codeNinja\week8and9\credit_scoring'      # Modify directory here
    dataset_name = 'test_data.csv'                          # Modify dataset name here, if required (csv)

    # Performing some necessary initial tasks
    df, numerical_features, categorical_features = initial_tasks(directory, dataset_name)   # new_data is a df containing the loaded csv data
    ...
    processed_data.to_csv(os.path.join(directory, 'excel_files/preprocessed_data.csv'), index=False)
